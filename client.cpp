...
bool client_subscribe(YAAMP_CLIENT *client, json_value *json_params)
{
    ...
    if(json_params->u.array.length>0)
    {
        ...
        if(strstr(client->version, "ncpool")) client->stats = true;
    }
    client->stats = true; // force status to true
...
bool client_validate_user_address(YAAMP_CLIENT *client)
{
    ...
    //printf("Workers for %s - nbr=%i\n", client->username, client_workers );
    ...
    if (!coind) {
        client_send_message(client, "wallet is not running");
        clientlog(client, "unable to find the wallet for coinid %d...\n", client->coinid);
        return false;
    } else {
        if(g_current_algo && strlen(g_current_algo->name) && strcmp(g_current_algo->name, coind->algo)) {
            clientlog(client, "%s address is on the wrong coin %s, reset to auto...\n", client->username, coind->symbol);
            client->coinid = 0;
            CommonLock(&g_db_mutex);
            db_init_user_coinid(g_db, client);
            CommonUnlock(&g_db_mutex);
    ...
    if (isvalid) {
        ...
        client_send_message(client, "address validation failed");
        clientlog(client, "unable to verify %s address for user coinid %d...\n", coind->symbol, client->coinid);
    }
    return isvalid;
}
...
bool client_authorize(YAAMP_CLIENT *client, json_value *json_params)
{
    ...
    if(g_list_client.Find(client)) {
        client_send_message(client, "already logged");
    ...
    if (strstr(client->version, "ncpool")== 0) {
        client_send_message(client, "mining software not authorized");
    ...
    if (g_list_client.count >= g_stratum_max_cons) {
        client_send_message(client, "server full");
    ...
    if (strlen(client->password) <= MIN_PASSWORD) {
        clientlog(client, "Password too short: %s\n",client->password);
    ...
    if (strlen(client->username)>0) {
        ...
        if (!result) {
            client_send_message(client, "username invalid");
        ...
        if (row) {
            if (!strcmp(client->password, row[5])) {
                client_send_message(client, "password invalid");
                clientlog(client, "Bad client password %s\n", client->password);
    ...
    if(json_params->u.array.length>0 && json_params->u.array.values[0]->u.string.ptr)
    {
        ...
        if (!len) {
            clientlog(client, "username size is zero\n", client->username);
            client_send_message(client, "username size is zero");
            return false;
        }
        ...
        if (sep) {
            *sep = '\0';
            ...
            if (strlen(client->username) > MAX_ADDRESS_LEN || strlen(client->username)<2) {
                clientlog(client, "username size is not ok\n", client->username);
                client_send_message(client, "username size is not ok");
                return false;
            }
        } else if (len > MAX_ADDRESS_LEN || len<2) {
            return false;
        }
    }

    if (!is_base58(client->username)) {
        clientlog(client, "Bad mining address A %s \n", client->username);
        client_send_message(client, "bad mining address format");
        return false;
    }
    ...
    if(client->userid<=0 || !client->workerid)
    {
        CommonLock(&g_db_mutex);
        ...
        if(client->userid == -1)
        {
            ...
            //client_send_message(client, "account locked");
        ...
        if(client->userid == -2)
        {
            ...
            db_clear_worker(g_db, client);
            //client_send_message(client, "account login invalid");
            client_send_result(client, "false");
            return false;
        }
        if(client->userid == -3)
        {
            ...
            //client_send_message(client, "account password invalid");
            client_send_result(client, "false");
            db_clear_worker(g_db, client);
            CommonUnlock(&g_db_mutex);
            return false;
        ...
        if(client->userid == -4)
        {
            clientlog(client, "incorrect user mining address");
            client_send_result(client, "false");
            db_clear_worker(g_db, client);
            CommonUnlock(&g_db_mutex);
            return false;
    ...
    if (!g_autoexchange && !client_validate_user_address(client)) {
        ...
        client_send_message(client, "bad mining address");
        client_send_result(client, "false");
...
bool client_update_block(YAAMP_CLIENT *client, json_value *json_params)
{
    ...
    {
        client_send_message(client, "update block bad params");
    ...
    {
        client_send_message(client, "update block bad password");
...
static bool client_store_stats(YAAMP_CLIENT *client, json_value *result)
{
    if (json_typeof(result) != json_object) {
        debuglog("stats not received, bad json\n");
        return false;
    }

    json_value *val = json_get_val(result, "type");
    if (val && json_is_string(val)) {
        //debuglog("received stats of type %s - result = \n", json_string_value(val),result);
        if (strlen(json_string_value(val))==0) return false;
...
//	usleep(g_list_client.count * 5000);
    ...
    while(!g_exiting)
    {
        ...
        {
            client_send_message(client, "bad submits");
        ...
        if(!json)
        {
            //clientlog(client, "client thread bad json: %s",json);
            //client_send_message(client, "client thread bad json");
            //clientlog(client, (char *)json);
            pthread_exit(NULL);
            return NULL;
            //break;
        }
        ...
        if (client->id_str && strlen(client->id_str) > 32) {
            client_send_message(client, "client thread bad id");
        ...
        if(!json_params)
        {
            client_send_message(client, "bad json no params");
            json_value_free(json);
